...
SOLRPDEVDATE=`curl -s "http://digcoll-dev.internal.library.cornell.edu/solr/digitalcollections2/select?q=id%3Ass\%3A$ASSET&fl=timestamp&wt=json&indent=true" | grep -E "\"timestamp\":\"[^\}]+" -o | cut -d '"' -f 4`
